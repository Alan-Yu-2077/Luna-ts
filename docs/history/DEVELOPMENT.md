...
| `v0.38.8` | 2026-07-18 | **One-click download button (Initiative 28)** — the README's download button linked to `/releases/latest`, which GitHub resolves EXCLUDING prereleases; against our pre-release the page 302'd to the plain releases list and `/latest/download/<asset>` 404'd (owner: "点击下载按钮后就自动开始下载" — it didn't). Three-part fix: (1) stable version-free `artifactName`s in electron-builder (`Luna-Setup-Windows-x64.exe`, `Luna-macOS-arm64.dmg`) so the filename never changes across releases (the old `Luna Setup 0.1.0.exe` baked in the version) — verified the dmg name locally; (2) the release is now a **normal** (non-prerelease) release so `/releases/latest` resolves to it, with the "early build / unsigned / Windows acceptance in progress" caveat moved into the release notes; (3) the README buttons (both langs) link directly to `/releases/latest/download/<stable-name>` → a click begins the download immediately, and the URL stays valid across future releases. Docs + build-config only; suite unchanged (1445). | _working tree_ |
| `v0.39.2` | 2026-07-23 | **Agent-only mode — the same brain with nothing but the chat box (OSS setup pass, 3/3)** — the wizard's six steps all assumed you wanted a body, and the front end always reserved ~62% of the room for a model stage that a new user has nothing to put in. New FIRST step asks which Luna you want: **the complete Luna** (Live2D + voice, 7 steps) or **just the agent core** (5 steps — the avatar/voice steps are GONE, not greyed out). The choice writes `LUNA_UI_MODE` to luna.env (new WIZARD_KEY, injected via `lunaConfig`), read by a new pure `resolveUiMode` (`?agent` → localStorage → config → full). In agent mode `app.ts` resolves no model, builds no Live2D sink, mounts no pack-drop, hides the Avatar settings pane + its rail icon and the Desktop-pet row, and re-parents the mood pip + Dream button into the chat header; `.agent-only:not(.collapsed)` gives the chat column the whole room. `modeValues` writes `LUNA_TTS_BACKEND` explicitly in BOTH directions so switching back to full is not left mute. +13 tests. Suite 1468 green; 4× tsc clean. | _working tree_ |
| `v0.39.1` | 2026-07-23 | **One-click installer gets a real progress bar (OSS setup pass, 2/3)** — the shell already reported `{stage, pct, bytesDone, bytesTotal, inFlight}`, but the wizard rendered it as one static line of text; against a 2–5.7 GB download followed by minutes inside pip, that is indistinguishable from a hang. New pure `provisionProgress` + `formatBytes` map the snapshot onto a bar: **bytes-weighted** while downloading a known total (`1.2 GB / 5.7 GB · 21%`), a **sweep** for the stages that have no byte total to weight against (extract / materialize / venv / validate) rather than a faked percentage, **pinned at the last pct** for a parked install (sweeping would claim it is still working), and green/red at ready/failed. `role=progressbar` + `aria-valuenow`; hidden entirely at `idle`. +9 tests. Suite 1457 green. | _working tree_ |
| `v0.39.0` | 2026-07-23 | **Setup wizard stops clipping itself (OSS setup pass, 1/3)** — dropping a voice pack appended eight rows to a card that had no `max-height` inside a `height:100vh; overflow:hidden` root, so the card grew straight past the viewport and was HARD-CLIPPED — at worst the Back/Next row itself became unreachable. The card is now clamped to `calc(100vh - 40px)` with `.wizard-step-body` as the only shrinking/scrolling region (dots, step title, status and actions pinned `flex:none`); the scan results collapse into one bounded `.wizard-pack` block with horizontal `label|value` rows instead of eight full-width form rows; the long walkthrough card became a `<details>` that auto-collapses once its step has content of its own (new pure `guideOpen`); and a step carrying scan results widens the card to `min(600px,96vw)` (`data-dense`) so it spends width before height. +3 tests. Suite 1451 green. | _working tree_ |
| `v0.38.7` | 2026-07-18 | **First real Windows-CI results: the release publish flag + 16 test-portability fixes (Initiative 28)** — the v0.38.6 tag ran the cloud release + the windows CI matrix for the first time, both red, both instructive. (1) **Release**: electron-builder detects a CI tag and tries to auto-PUBLISH to GitHub → dies for want of `GH_TOKEN` (we publish separately via `action-gh-release`). Fixed with `--publish never` on `pack:win`/`pack:mac`; added the `author` field electron-builder warned about. (2) **CI (windows-latest)**: 16 fails, **every one a test-harness portability issue, zero product bugs** — the win install/compile:server:win/pack all PASSED. Categories: POSIX-path fixtures that don't match `node:path`'s win32 output (`backend` ×4 → build fixtures with `join()`), `process.platform`-defaulting assertions (`voicePack` ×3, `vecRuntime`, `registry` default-mount → pass an explicit platform), the win32 shell-unmount asserting shell present, the default kill's taskkill branch spawning a real process in a unit test (`supervisor` ×2 → inject `killFn`), `/bin/echo` absent on win (`shellCore` argv → skip), and the **`sql` ×4** which looked like a real DB failure but were an `afterEach` **EBUSY** — Windows holds a lock on the WAL/SHM files for a moment after `closeDb`, so the immediate `rmSync` of the temp dir throws (the DB itself: WAL on, migrations applied; best-effort cleanup now). Mac suite unchanged (1445 green). | _working tree_ |
| `v0.38.6` | 2026-07-17 | **Cloud release workflow (Initiative 28)** — NEW `.github/workflows/release.yml`: on a `v*` tag (or manual dispatch) a `windows-latest` job builds the NSIS `Luna Setup <v>.exe` and a `macos-latest` job builds an unsigned arm64 `Luna-<v>-arm64.dmg`, each attached to a **pre-release** GitHub Release via `softprops/action-gh-release`. New `pack:mac` script. Native-runner builds bypass the flaky-network local packaging; fires only on a tag, never an ordinary push. Both installers built + verified locally (`Luna Setup 0.1.0.exe` x64, `Luna-0.1.0-arm64.dmg` 129 MB APFS). Unsigned by design; no keys in the bundle. Suite 1445. | _working tree_ |
| `v0.38.5` | 2026-07-17 | **Server parity for Windows (Initiative 28, 6/6 — closes the code)** — the `shell` tool unmounts on win32 with a log (its spawner is `/bin/zsh`; the argv verify tools stay); sqlite-vec resolution is platform-aware (win32 → `LUNA_SQLITE_LIB` override, no dead unix probes); the secret blocklist learns the Windows browser/cred dirs (`AppData` Chrome/Firefox/gcloud) + the shell path-scan learns drive/`%VAR%` tokens; `editCore` retries a win32 EPERM/EBUSY rename (Windows locks open files); a loopback-only `POST /shutdown` + a tested desktop `postShutdown` helper make a graceful win32 shutdown reachable (SIGTERM is never delivered there). The desktop quit→POST wiring + real-machine checks are PENDING the x64 PC. +17 tests. Suite 1445 green. | _working tree_ |
| `v0.38.4` | 2026-07-17 | **Windows TTS 整合包 — the mac-buildable half (Initiative 28, 5/6)** — a bundled, sha256-pinned `7zr.exe` (602 KB LGPL, fetched by `fetch-7zr.ts`, shipped as a win extraResource) + a `sevenZipCandidates` ladder (bundled → `%ProgramFiles%\7-Zip` → PATH) so a stock machine never needs a system 7-Zip (7-Zip's installer doesn't add itself to PATH — the #1 predicted install failure); a `checkoutFfmpeg` probe preferring the 整合包's own ffmpeg. Verified the win package bundles `7zr.exe` beside `luna-server.exe`. +7 tests. The real 8.19 GB download / archive-structure / api_v2-speech verification is PENDING the x64 PC. Suite 1428 green. | _working tree_ |
| `v0.38.2` | 2026-07-17 | **Windows packaging + first boot (Initiative 28, 3/6)** — electron-builder `win` NSIS+dir target (x64, `.ico` from the 1024² png), per-platform `extraResources` (verified electron-builder MERGES root + platform — the win package carries `luna-server.exe` + migrations/persona/web), `app.requestSingleInstanceLock` + a `serve.ts` listen-error handler (no EADDRINUSE crash on a double-launch), `pack:win` + `scripts/app.ts --win`. A real `Luna Setup 0.1.0.exe` (x64 PE32, Nullsoft) built FROM the mac; mac repacked + smoked (no regression). The five real-machine boot checks are PENDING the x64 PC. (Follow-up `b17e1b5`: `pack:win` made config-driven so the installer is x64, not the build host's arm64.) +1 test. Suite 1421 green. | _working tree_ |
| `v0.38.1` | 2026-07-17 | **CI Windows matrix + portable web build (Initiative 28, 2/6)** — `ci.yml` runs the full suite + all typechecks on `ubuntu-latest` AND `windows-latest` (fail-fast off; apt gated to Linux, choco ripgrep on Windows), adds a portable-web-build proof + a `compile:server:win` cross-compile step; the web build's `rm/cp/mkdir` shell one-liner becomes `scripts/build.ts` (node:fs) with a tested `stageWebAssets` helper. Cross-compiling `luna-server.exe` from the mac verified locally (real PE32+ binary). The windows CI leg's first real run is pending the next push to the public repo. +3 tests. Suite 1420 green. | _working tree_ |
| `v0.38.0` | 2026-07-17 | **Windows process/path substrate (Initiative 28, 1/6)** — the mechanical win32-semantics layer everything else in the Windows initiative rides, all platform-branched so the mac daily driver is byte-equivalent. (1) **Supervisor tree-kill seam**: `child.kill()` on win32 is TerminateProcess on the DIRECT child only — a `bun scripts/dev-all.ts` or a pip/python tree orphans grandchildren holding the port + DB lock; new injectable `killFn` defaults to `taskkill /pid N /T /F` on win32 and SIGTERM-then-SIGKILL escalation on POSIX (closes the no-escalation gap on mac too). Pure `killPlan(platform,pid)` unit-tests the argv off-platform. (2) **PATH assembly** extracted to a new testable `childEnv.ts` (`serverBinName`/`pathKeyFor`/`childPathValue`): joins on the platform delimiter (`;` win32) not a hardcoded `:`, appends the homebrew dirs on darwin only, and writes back to the env's own case-variant key (`Path` vs `PATH`) so the child never gets two conflicting vars. (3) **`luna-server.exe` chain**: `serverBinName(platform)` + a `compile:server:win` cross-compile script (`--target=bun-windows-x64`). (4) **`windowsHide: true`** on every spawn/spawnSync (no console-window flash). (5) **Discovery**: win32 last-resort interpreter is `python` not `python3`; bun discovery honors `USERPROFILE` + `bun.exe`; `validateRuntimeDir`/`startCommand` learn the `.venv\Scripts\python.exe` layout + a PowerShell copy-paste form. +18 tests. Suite 1417 green. | _working tree_ |
| `v0.37.17` | 2026-07-16 | **Location un-frozen (owner-reported)** — weather stopped following the machine: v0.33.0 persisted the first CoreLocation fix INTO `LUNA_LAT_LON`, and the next boot read that self-written cache as a manual operator pin ("operator's word is final") → returned null → never asked CoreLocation again. One fix, frozen forever (the owner's env held a months-old coordinate) — the exact inverse of the pre-persist behavior, where every boot re-acquired and even small moves registered. Three-surface fix: (1) the auto cache moves to `LUNA_LAT_LON_AUTO` — chain is now manual pin > fresh CoreLocation > AUTO cache (backstop only, never blocks) > timezone city; (2) the desktop shell re-polls CoreLocation every 10 min (async — the sync exec would freeze windows), and a real move (>~55 m, `movedBeyond`) is pushed over a new `lunaPet.onGeoFix` preload channel → renderer forwards as `client.geo` → server runtime location beats the env fallback live, no restart; (3) the web's one-shot `getCurrentPosition` (page-load snapshot, re-sent forever on reconnect) becomes `watchPosition` — the browser announces movement itself, `setGeoFix` keeps reconnect re-sends at the NEWEST fix. Manual pins still silence everything. Server untouched (precedence was already right — the upstream just never sent fresh coordinates). +12 tests. Suite 1399 green. | _working tree_ |
| `v0.37.16` | 2026-07-15 | **Two more blockers the completed re-audit confirmed: no-compiler + GitHub-tarball time bomb** — the re-audit finished (48/51 agents; its #1 finding independently re-derived v0.37.15's exact Python-floor fix, validating it). Two survivors past v0.37.15: (1) **jieba_fast AND pyopenjtalk are sdist-only on PyPI** (0 wheels, every version — verified) → pip must compile them, so a clean machine with no Xcode CLT / build-essential passes every check, downloads ~1.6 GB, then dies at the venv stage; the reference machine had CLT, so the e2e never saw it. Added a `hasCompiler()` preflight gate (mac: `xcode-select -p`, no install-popup; Linux: `cc --version`) with a `COMPILER_HINT`. (2) The **GitHub source tarball was sha256-pinned** — but GitHub does not guarantee byte-stability of auto-generated tarballs (its Jan-2023 gzip change flipped every such hash and broke Homebrew + Go). The pin was a time bomb: the day GitHub recompresses, every new install hard-fails at the first artifact. Removed the pin (a wrong body isn't valid gzip → `tar -xzf` fails loudly), and a failed extract now deletes the cached archive so retry re-downloads (no permanent wedge). +4 tests. Suite 1388 green. | _working tree_ |
| `v0.37.15` | 2026-07-15 | **Re-audit fixes: Python 3.9 rejected, torch ceilinged, pack-drop switches the live voice, honest health badge, no orphaned installers** — the re-run 6-lens audit (31 agents, 16 findings, 26 verdicts) landed real ones. (1) A clean Mac with only the system `python3` (3.9.6) built a 3.9 venv that silently resolves DOWNGRADED torch 2.8 / numpy 2.0 — never validated against the 2025 code tag; the floor is **3.10** now (modern torch/transformers/tokenizers/numba all dropped 3.9, confirmed on PyPI), `/usr/bin/python3` dropped from the probe. (2) torch/torchaudio/torchcodec were **unpinned** → the next major lands on every fresh install; **ceilinged** to the proven combo (`torch>=2.4,<2.14` resolves to 2.13, verified). (3) An in-app pack drop armed managed mode + started api_v2 but never set `LUNA_TTS_BACKEND=http` nor reloaded — the cloned voice ran **unused** behind the browser sink while the chip said "✓ swapped"; now switches the backend + reloads. (4) The wizard health badge went green on `res.ok`, which is TRUE for the `starting`/`gave-up` 200s — now reads the body (green only on real ready, a distinct "warming"). (5) pip/torch installer children (10-30 min) were orphaned on quit; tracked + killed. Recorded known limit: `LUNA_TTS_HF_MIRROR` only covers HuggingFace, not the GitHub tarball / nltk / lid.176. +6 tests. Suite 1384 green. | _working tree_ |
| `v0.37.14` | 2026-07-15 | **Audit hardening: mirror-safe checksums confirmed, weight picker, ffmpeg memoized** — while the re-audit ran (its first launch died to network flaps mid-run), three of its findings were verified and closed by hand. (1) The new sha256 gate does NOT break the CN mirror: `hf-mirror.com` serves byte-identical files (its `config.json` hashes to the same `3d57de2f…` the manifest pins), so a mirror user is not hard-failed by the checksum — the feared regression is not real. (2) `adoptInstalledPacks` picked a pack's weight by "first non-dotfile", so a stray `readme.txt` sorting before the `.ckpt` would be yaml'd as the model — now `pickWeight` matches by EXTENSION and skips AppleDouble (`._`) forks (pure, +4 tests). (3) `findFfmpeg()` spawned `ffmpeg -version` on the UI thread on every supervisor (re)creation → memoized. Suite 1381 green. | _working tree_ |
| `v0.37.13` | 2026-07-14 | **The clean room wasn't clean — English synthesis was broken for every new user** — v0.37.12's end-to-end "proof" ran on the reference machine, which has had `~/nltk_data` since June and pyopenjtalk's dictionary cached. Re-running the synth under a **fake `$HOME`** — the only way to simulate a machine that has never run GPT-SoVITS — showed what a new user actually gets: `g2p_en → LookupError: Resource 'cmudict' not found` (**nltk does not even auto-download it**; English G2P dies outright) and pyopenjtalk fetching its 22.6 MB dictionary mid-conversation. The corpora now ship as **manifest artifacts** (checksummed, resumable, in the progress bar) rather than an opaque `nltk.download()` at install time, `NLTK_DATA` is set on the api_v2 child (nltk searches `$HOME` and the venv, never the checkout), pyopenjtalk's dict is force-fetched inside the install, and `validating` treats the English corpus as load-bearing. **Every one of the 12 artifacts now carries a sha256** — the Windows 整合包's taken from HuggingFace's LFS etag, cross-checked against a file hashed by hand. **Verified under a fake `$HOME` AND a minimal PATH: api_v2 boots and synthesizes English (4.84 s, RMS 4198) and mixed 中英 (3.60 s, RMS 3793).** Suite 1377 green. | _working tree_ |
| `v0.37.12` | 2026-07-14 | **The one-click voice install is REAL — proven by running it, which found six more blockers (owner-demanded)** — the owner refused to accept HEAD-checks as proof. Correct: an actual clean-room install (1.57 GB → venv → api_v2 → synth) found the button could still **never** have worked. Six blockers, each invisible to code review: (1) `python3` was hardcoded, the reference machine's own is **3.14** (no torch wheels) and preflight never checked → fail AFTER 1.6 GB; (2) upstream `requirements.txt` pins **`numba==0.56.4`, whose installer REFUSES python 3.11** — the pip command could not succeed on the python GPT-SoVITS itself recommends; (3) the code tag `20240821v2` **crashes on import** with any modern torch (`NameError: Tuple`) — the reference instance actually runs **`20250606v2pro`** (proven by hashing its `patched_mha_with_cache.py` against every release); (4) `matplotlib`, `split-lang` and **`gradio`** (load-bearing, despite looking like WebUI weight) were missing; (5) gradio's latest pulls `huggingface-hub` 1.x, which **transformers rejects at import**; (6) `torchcodec` + the **system `ffmpeg` binary** are required to decode the reference clip — without them every synth 400s AFTER a "successful" install. All six fixed; preflight now gates on python AND ffmpeg BEFORE downloading. **Verified: api_v2 boots from the provisioned runtime and synthesizes 5.64 s of real audio (32 kHz, RMS 4151).** Also: the browser fallback voice picked the OS default — a **male** voice behind a girl's avatar (`voicePick`: language-matched, female-preferred, word-boundary matched). +14 tests. Suite 1369 green. | _working tree_ |
| `v0.37.11` | 2026-07-14 | **CI has been red on every push since it first ran — two tests exec zsh, ubuntu has none (owner-asked)** — every public push failed the `Test` step in ~40s: the two `realSpawner` zsh-path tests (`$()`-interpretation proof, process-tree kill) spawn `/bin/zsh -lc` (`shellCore.ts:90`) and ENOENT'd in <1ms on the runner. The mac dev machines always had zsh, so it never failed locally — the workflow was red from its very first public run and nobody was looking. Fix: the runner installs **zsh** (the invariants now actually execute on Linux), the two tests gain a `skipIf(!/bin/zsh)` guard (zsh-less contributor machines skip instead of failing), and the workflow gains the **desktop typecheck step it never had** (it predates `packages/desktop`). Recorded, not fixed: the shell TOOL itself hard-requires `/bin/zsh` at runtime — a zsh-less Linux install has a broken `shell` tool; a bash fallback is a deliberate separate decision. Suite 1346 green (incl. v0.37.10's pin). | _working tree_ |
...
| `v0.37.5` | 2026-07-14 | **Smoke web-port isolation** — the packaged smoke wedged (no probe, no exit) the first time it ran alongside a LIVE instance: `WEB_PORT` (5177) is pinned with no smoke override, so `startWebHost` hit EADDRINUSE against the running app. `DESKTOP_WEB_PORT = SMOKE ? 5178 : 5177` (+ `LUNA_DESKTOP_WEB_PORT` override), mirroring the existing `SERVER_PORT` smoke isolation (8790). Verified: app smoke green (`ok:true`, own 8790 sidecar, 19 server rows) and the wizard voice-step smoke green (provision button + new copy render) — both WHILE the owner's instance stayed up untouched. | _working tree_ |
| `v0.37.4` | 2026-07-14 | **兜底阶梯 + 首句保护 + 文案收口 (Initiative 27, 5/5 — closes the initiative)** — the two-rung ladder lands where utterances actually die: `WebAudioSink` gains `onUnspoken` — a hard-failed synth OR the 60s mute window hands the words (serialized) to a lazy **browser-voice fallback** instead of silently dropping them; retryable 502/504s during a managed swap fall back for THAT sentence while every next one retries http (recovery self-promotes); barge-in aborts stay silent by design. This IS the first-utterance protection for post-boot restarts. Copy pass: the wizard voice guide rewritten around the one-click flow (no more "clone the repo / run the command in a terminal"), the runtime picker demoted to "Advanced", and `docs/SETUP.md`'s voice section rebuilt (one-click + pack + managed first; docker/BYO as the advanced appendix). +4 ladder tests (injected fetch, no AudioContext). Suite 1335 green. **Initiative 27 code-complete; `LUNA_TTS_PROVISION` stays off pending per-OS real-download validation.** | _working tree_ |
| `v0.37.3` | 2026-07-14 | **拖包换音色 — managed hot-swap (Initiative 27, 4/5)** — dropping a weights pack now BECOMES the voice, no terminal anywhere. Managed mode: the install handler generates the yaml against the **managed checkout** (new `resolveManagedCheckout` — works for the FIRST pack, before any yaml exists), then **hot-swaps** the voice child (`swapManagedTts`: stop old → adopt-if-external → fresh supervisor on the new argv → bounded port wait) and returns `{managed, ready}` instead of a command; the wizard's command block is naturally absent (it renders only when a command is returned). BYO/unmanaged keeps the legacy command path byte-identical. NEW: **drop a pack onto the RUNNING app** (`ui/packDrop.ts`, desktop-only) — scan → one-tap confirm chip → install → swap; single-candidate packs auto-pick (the bilibili shape), ambiguous packs route to the wizard. +10 tests. Suite 1331 green. | _working tree_ |
| `v0.37.2` | 2026-07-14 | **一键下载并部署 GPT-SoVITS (Initiative 27, 3/5 — 标准 1)** — the wizard's voice step gains ONE button that downloads, deploys, and validates a complete GPT-SoVITS runtime. NEW `ttsProvision.ts`: a staged, **resumable** installer (preflight disk → download with `.part` + Range resume → extract → materialize → venv → validate → `provision.json ready`) over injected seams; the mac recipe is the owner's verified lean set (code `20240821v2` + roberta + hubert + G2PW + lid.176 ≈ **2 GB**, no gsv pretrained — custom voices bring their own weights); Windows = the official **整合包** (5.7 GB 7z, system-7-Zip extraction with a manual-placement fallback). CN reachability: `LUNA_TTS_HF_MIRROR` host override + bcebos-native G2PW. `luna:provision-start/-status` IPC (polled), wizard button + bilingual staged copy, `LUNA_TTS_PROVISION` preview flag (default off). ttsRuntime: **no pack → no launch** (Open Q4: browser voice until the first pack) + the 整合包's embedded `runtime\python.exe`. +15 tests. Suite 1321 green. | _working tree_ |
| `v0.37.1` | 2026-07-14 | **冷启动加载页 — managed-wait semantics (Initiative 27, 2/5 — 标准 2)** — the SHIPPED boot gate learns that "not answering yet" can mean "Luna spawned it, wait". `warmUpTts`: the warm-synth racer now **retries** while health says `starting`/`restarting` (before, a refused synth failed the gate seconds into a managed cold start — the 'unavailable' bug's synth-side twin); `gave-up` fails fast (first health AND mid-poll) instead of burning the 120s deadline; a `restarting` stage label lands; timings injectable. `onStatus` now also carries the raw health state, and `app.ts` uses it to **hide Skip for the first ~20s of a managed wait** (revealed on time and instantly when warm-up resolves — v0.35.6 never-strand rule). BYO 502 fast-path, already-warm reload, and unmanaged synth fast-fail all byte-identical (test-pinned). NEW `warmUpTts.test.ts` (7). Suite 1306 green. | _working tree_ |
| `v0.37.0` | 2026-07-14 | **Managed TTS supervisor seam (Initiative 27, 1/5)** — the never-spawn boundary gets its escape hatch: with `LUNA_TTS_MANAGED=1`, Luna spawns + supervises the GPT-SoVITS api_v2 child herself (new `ttsRuntime.ts`: loopback-only, provisioned-runtime-over-BYO precedence, newest-pack-yaml voice pick, **argv** launch form — never a shell string; a third `createSupervisor` in `main.ts` with adopt-if-listening, SMOKE no-op, and both quit paths killing it). `/api/tts/health` now answers `{state:'starting'|'restarting'|'gave-up'}` (200) when the upstream is unreachable but the managed child is in flight — the boot gate's cue to WAIT — while unmanaged keeps the bare 502 (BYO semantics intact). `LUNA_TTS_MANAGED` in ENV_TEMPLATE + WIZARD_KEYS. Default off; flag off = byte-identical to v0.36.10. +19 tests (ttsRuntime 14, serve health 4, +1). Suite 1299 green; 4× tsc clean. | _working tree_ |
...
### `v0.39.2` — 2026-07-23 — Agent-only mode: the same brain with nothing but the chat box (OSS setup pass, 3/3)
...
- NEW `web/src/uiMode.ts` (+ `uiMode.test.ts`, 6 tests): pure `resolveUiMode({storage, config, search})` → `'full' | 'agent'`, precedence `?agent` (a browser-only preview hatch, like `?pet`/`?wizard`) → localStorage `luna:ui-mode` → `lunaConfig().uiMode` → `'full'`; any unrecognised value falls back to full rather than to a stripped front end. `lunaConfig.ts`'s `LunaConfig` gains `uiMode?`.
- `web/src/ui/setupWizard.ts`: `WizardStepSpec['id']` gains `'mode'`; `wizardSteps(mode = 'full')` is now a function OF the mode — full returns 7 steps, agent returns 5 with `avatar`/`voice` absent (not disabled). New exported pure `modeValues(mode, voiceBackend)`. The renderer gains a `mode` branch (two `.wizard-mode-card` buttons whose click IS the Next for that step) and a `selectMode` that rebuilds the step table + nav; `steps`/`nav` moved from `const` to `let`. Submit merges `modeValues` LAST so it beats a stale `LUNA_TTS_BACKEND` from the voice radio. Prefill adopts a stored `LUNA_UI_MODE`, hydrates against the FULL step table (so an agent re-run doesn't drop the stored voice URL), and now only adopts a stored TTS backend of `browser`/`http`.
- `web/src/ui/layout.ts`: `LayoutRefs` gains `chatHeader`, `avatarTab`, `avatarRailBtn` (pure additions — layout stays a builder, app.ts decides).
- `web/src/app.ts`: agent mode adds `.agent-only` to the root, skips model resolution + `createPixiLive2DSink` + `applyEmptyState` entirely, skips `mountPackDrop`, hides the Avatar tab/rail button and the Desktop-pet row, and re-parents `moodPip` + `dreamBtn` into the chat header (they were absolutely positioned inside the model stage).
- `web/src/ui/theme.css`: `.wizard-mode-cards` / `.wizard-mode-card` skin; `.agent-only:not(.collapsed)` hides the model stage and gives the chat panel `max-width: 900px` centred; the re-parented pip/dream button are restyled static.
- `web/src/ui/setupCopy.ts`: +6 bilingual keys (`step.mode.*`, `guide.mode`).
- desktop: `WIZARD_KEYS` += `LUNA_UI_MODE` (outside the whitelist it would be silently dropped); `currentLunaConfig()` + the preload's `lunaConfig` type carry `uiMode`; `ENV_TEMPLATE` documents it; `smokeSetupProbe` asserts 7 dots and clicks `.wizard-mode-full` before walking to the voice step.
- Docs: `README.md` / `README.zh-CN.md` onboarding bullets rewritten around the two modes; `docs/SETUP.md` gains a mode table; `.env.example` documents `LUNA_UI_MODE`.
- Tests: +6 `resolveUiMode`, +3 `modeValues`, +3 `wizardSteps` (full order, agent drops both resource steps, every step's title resolves in zh+en), +1 bilingual guide coverage. Suite 1457 → 1468.

Inference:

- This is the first time the front end is a CHOICE rather than a constant, and the shape of that choice matters: agent mode REMOVES the two resource steps instead of disabling them, because a step you cannot use is still a step you have to read — the whole point is that the fast path is actually fast. Same reasoning behind hiding the Avatar pane and the pet row rather than leaving dead controls.
- `modeValues` writing `LUNA_TTS_BACKEND` in both directions is the non-obvious half. `collectValues` drops empty fields precisely so a skipped step can't clobber luna.env (v0.37.8), and `mergeEnvFile` then preserves what is stored — which means an agent run's `none` would SURVIVE a later switch back to full and hand the user a mute companion with no visible cause. The mode has to overwrite what the mode set. The prefill guard is the same bug from the other side: a stored `none` must not be adopted as the voice step's backend.
- Keeping the model stage alive under `.collapsed` is what lets agent mode coexist with the v0.25.1 companion collapse instead of forbidding it: the SpeechStackView is mounted inside that stage, so an unconditional `display:none` would have made collapsing agent mode show nothing at all. Expanded, the stage costs zero width — which is the actual requirement.

### `v0.39.1` — 2026-07-23 — One-click installer gets a real progress bar (OSS setup pass, 2/3)
...
- `web/src/ui/setupWizard.ts`: new exported types `ProvisionSnapshot` / `ProgressView` and pure functions `formatBytes(n)` and `provisionProgress(snapshot)`. The provision box gains a `.wizard-progress` track + `.wizard-progress-fill` (`role=progressbar`, `aria-valuemin/max`, `aria-valuenow` only when determinate) and a `.wizard-progress-detail` line; `paint()` now reads `provisionProgress(...)` instead of splicing a bare `pct` into the status copy, and reads the snapshot's numbers through a local cast-free `num()` helper.
- `web/src/ui/theme.css`: `.wizard-provision` (a real column layout — it had no styles at all before), `.wizard-provision-status`, and the bar: track, transitioned fill, `data-tone` ok/error colours, and a `.indeterminate` sweep keyframe.
- `web/src/ui/setupWizard.test.ts`: +9 tests — `formatBytes` unit scaling and its 0/negative/NaN floor, and the `provisionProgress` truth table (idle hidden, bytes-weighted download, no-byte-total stages sweeping, parked determinate, a parked later stage NOT resurrecting the finished download's byte label, ready/failed tones, and pct clamping). Suite 1451 → 1457.

Inference:

- The data was always there — `provisionStatus()` has returned `bytesDone`/`bytesTotal` since v0.37.2 — so this is a rendering gap, not a plumbing one. What it costs is trust: the two longest stretches of the install (a multi-GB download and a pip/torch venv build) are exactly where a user decides whether the app is working or dead, and a frozen line of text says "dead".
- The interesting rule is the one about NOT showing progress. A percentage the shell cannot actually compute (extract, venv, validate) would be a lie that erodes the bar's meaning for the stage that can compute one; and a parked install that keeps sweeping claims work that stopped when the app quit. Sweeping only where the total is unknown, and freezing where nothing is running, keeps the bar's two signals — *how far* and *is it alive* — independently honest.

### `v0.39.0` — 2026-07-23 — Setup wizard stops clipping itself (OSS setup pass, 1/3)
...
- `web/src/ui/theme.css`: `.setup-card.wizard` gains `max-height: calc(100vh - 40px)` and `.luna-app.setup` gains `padding: 20px 0`; `.wizard-step-body` becomes the sole flexible/scrolling region (`flex:1 1 auto; overflow-y:auto; overscroll-behavior:contain`, negative-margin scrollbar gutter) with the title/sub/dots/step-title/status/actions pinned `flex:none`. New `.setup-card.wizard[data-dense='1']` widens to `min(600px,96vw)`; new `.wizard-pack` block skin (horizontal `label|value` rows, ellipsised filenames, height-capped command `<pre>`); `.wizard-guide` reworked as a `<details>` skin with a rotating chevron summary.
- `web/src/ui/setupWizard.ts`: new exported pure `guideOpen(stepId, hasResults)` — only `voice`/`avatar` may auto-collapse their walkthrough, every other step keeps it open because there the guidance IS the step. The guide element is now a `<details>` + `<summary>` (class `.wizard-guide` unchanged, so the packaged smoke's selector still matches); the voice step's scan output (3 pick rows, transcript, prompt-lang, runtime row, install button, command block) is re-parented from the step body into one `.wizard-pack` container; the renderer sets/clears `card.dataset.dense`; a successful avatar install now sets `avatarInstalled` and re-renders (status re-applied after, the v0.35.3 voice-install pattern) so its walkthrough collapses too.
- `web/src/ui/setupCopy.ts`: +2 bilingual keys — `wizard.guide.summary`, `step.voice.pack.title`.
- `web/src/ui/setupWizard.test.ts`: +3 tests (`guideOpen` truth table — resource steps collapse on results, every step arrives open, key steps never collapse). The existing zh/en parity test covers the new copy keys automatically. Suite 1448 → 1451.

Inference:

- The clipping was structural, not cosmetic: `.luna-app` is `height:100vh; overflow:hidden` (v0.36.5 — "the app is a FIXED single screen, never a scrolling page"), so ANY content that outgrows the viewport is silently cut rather than scrolled. The wizard inherited that root and had no clamp of its own, which meant the v0.35.3 pack-drop flow could put its own Back/Next past the cut line. Clamping the card and moving the scroll INSIDE it keeps the v0.36.5 no-page-scrollbar rule intact while making the wizard's chrome unconditionally reachable — the same "never strand the user" property v0.35.6 established for the escape hatches.
- The density work attacks the cause rather than the symptom: the guidance card and the scan results were competing for the same column, and the guidance never yields on its own. Making it a `<details>` that stands down once the step produces content means the space goes to whatever the user must act on next, and the wider `data-dense` card spends the cheap axis (width) before the expensive one (height).

### `v0.38.5` — 2026-07-17 — Server parity for Windows (Initiative 28, 6/6 — closes the code)

Status:

- working tree (all code + unit-proven; the desktop quit wiring + real-machine checks PENDING the owner's x64 PC)

Fact:

- `tools/registry.ts`: new `verifyTools` (typecheck/run_tests/lint — argv `bun`/`bun x`, cross-platform) split out of `shellTools`; new pure `shellSupported(platform)` (false on win32 — the spawner is `/bin/zsh -lc`); `withShell(base, platform?)` unmounts `shell` on win32 (logged) while keeping the verify tools, rather than shipping a PowerShell spawner with unvetted deny semantics (initiative Open Q #1 → decided toward the safe posture; a native win32 shell is a future, deny-reviewed opt-in). +2 tests.
- `memory/recall/vecRuntime.ts`: `resolveSqliteLib` gains a `platform` arg; on win32 there are NO default candidates (Windows' system winsqlite3.dll has extension loading compiled out, like macOS system SQLite) — the mechanism is the `LUNA_SQLITE_LIB` override pointing at a sqlite-vec-compatible sqlite3.dll; absent → the TS-cosine fallback (unchanged). No dead `.dylib`/`.so` probes on win32. +2 tests.
- `tools/workspace.ts`: the secret-dir blocklist gains the Windows equivalents under `%USERPROFILE%` — `AppData\Local\Google\Chrome\User Data`, `AppData\Roaming\Mozilla\Firefox`, `AppData\Roaming\gcloud` (case-folded on win32). These are load-bearing on win32 (the file tools are mounted there). +1 test.
- `tools/builtin/shell.ts`: `blockedPathInCommand`'s path-token regex learns drive paths (`C:\…`) and `%VAR%`-prefixed paths — defense-in-depth for a future win32 shell (the tool is unmounted on win32 today).
- `tools/editCore.ts`: new pure `renameAtomic(from, to, deps?)` — on win32 retries a locked-target rename (EPERM/EBUSY/EACCES) up to 2× with backoff (Windows locks open files where POSIX doesn't); POSIX rethrows immediately (unchanged). `atomicWrite` uses it. +4 tests.
- NEW `server/src/shutdownRoute.ts` + `.test.ts` (5 tests): pure `shouldHonorShutdown(method, pathname, bindHost)` — POST `/shutdown` honored ONLY on a loopback-bound server (a LAN-exposed instance is never remotely killable). `server/src/main.ts` hoists a `triggerShutdown` set by both key/no-key branches to the graceful path, and the `Bun.serve` fetch handler routes POST `/shutdown` → 200 + async graceful shutdown (so the shutdown dream runs on win32, where SIGTERM is never delivered).
- NEW `desktop/src/gracefulShutdown.ts` + `.test.ts` (3 tests): `postShutdown(port, timeoutMs, fetchFn?)` — best-effort, bounded, never rejects; the ready-to-wire helper the desktop calls before the tree-kill.

Inference:

- The always-erroring win32 `shell` (mounted but its `/bin/zsh` spawner ENOENTs) is replaced by an honest posture: absent + logged, with the argv verify tools (which never touch the shell) still available. This keeps the always-on + deny-regex LD intact on the platforms that have a vetted shell, and suspends it — explicitly, on one platform — rather than silently violating its deny-regex premise with an unreviewed shell.
- The graceful-shutdown pieces (server endpoint + desktop helper) are both present and tested, but deliberately NOT auto-wired into the Electron quit path: getting the `before-quit`/`window-all-closed` async timing right on win32 (preventDefault, the tree-kill fallback, no hung quit) requires a Windows machine in the loop — exactly the "reference machine is the worst witness" lesson. Wiring it blind would be the anti-pattern. So win32 quit currently skips the shutdown dream (a DEGRADED nicety, not data loss — SQLite WAL is durable); the endpoint makes the graceful path reachable (a manual `curl -X POST /shutdown` runs it today) and the real-machine session completes the wiring.
- HONEST GAP (real-machine, owner's PC): the desktop quit→POST wiring; sqlite-vec loading on win32 with a real dll (LUNA_SQLITE_LIB + ABI vs the cross-compiled bun sidecar); a shutdown-dream row + clean WAL after a real quit. The code is complete and unit-proven; these are the observations only Windows can make.

### `v0.38.4` — 2026-07-17 — Windows TTS 整合包: the mac-buildable half (Initiative 28, 5/6)

Status:

- working tree (extractor ladder + ffmpeg probe + 7zr bundling PROVEN on the mac; the real 8.19 GB download + api_v2 speech PENDING the owner's x64 PC)

Fact:

- NEW `packages/desktop/scripts/fetch-7zr.ts`: downloads the official standalone `7zr.exe` from 7-zip.org into `packages/desktop/bin`, sha256-pinned (`56b8cc9f…`, 602 112 bytes) so a changed/MITM'd binary is rejected; idempotent (skips when present + verified). `package.json` gains `fetch:7zr`; `pack:win` runs it first. `bin/7zr.exe` is gitignored (produced, not committed — same discipline as `luna-server.exe`).
- `package.json` win `extraResources` gains `{ from: bin/7zr.exe, to: 7zr.exe }`. Verified the win-unpacked/NSIS package bundles `7zr.exe` alongside `luna-server.exe` + migrations/persona/web.
- `ttsProvision.ts`: new pure `sevenZipCandidates(env, platform)` — ordered `LUNA_7ZR_DIR/7zr.exe` (the bundled extractor) → `%ProgramFiles%\7-Zip\7z.exe` + `(x86)` → PATH `7z`/`7za`/`7zr`; the `.7z` extract path now iterates it instead of the hardcoded three. New pure `checkoutFfmpeg(checkout, platform, exists)` — prefers the 整合包's own `runtime\ffmpeg.exe`/`ffmpeg.exe` on win32, null elsewhere. +7 tests.
- `main.ts`: sets `LUNA_7ZR_DIR` (resourcesPath packaged / `bin` dev, same split as serverBin) before `runProvision`; the managed TTS supervisor prefers `checkoutFfmpeg(rt.checkout, …)` over system ffmpeg discovery.

Inference:

- 7-Zip's installer not adding itself to PATH was the #1 predicted stock-machine failure of the whole Windows install (an installed-7-Zip box would still fail to unpack). Shipping the 600 KB extractor removes it categorically — the bundled candidate always leads, so the user never has to touch 7-Zip. This is the resolution of the initiative's Open Q #2, decided toward "bundle it".
- Ordering these as pure, tested seams (candidates as data, not spawns) means the real-machine session only has to confirm the archive's INTERNAL shape (stripPrefix, the exact ffmpeg subpath, whether nltk/G2PW ship inside), not re-derive the extractor plumbing.
- HONEST GAP / explicitly NOT done here (needs the x64 PC + a real 8.19 GB download): the etag-derived sha256 pin vs the real bytes, the `stripPrefix: 'GPT-SoVITS-v2pro-20250604'` vs the real top-level, the 整合包 contents inventory (ffmpeg's exact path — the `checkoutFfmpeg` locations are the likely ones, skipped-if-absent — plus nltk/G2PW presence), embedded-python api_v2 boot, and audible speech. Those are v0.38.4's real-machine acceptance, deferred to the owner.

### `v0.38.2` — 2026-07-17 — Windows packaging + first boot (Initiative 28, 3/6)

Status:

- working tree (Windows installer built + structurally verified; real-machine boot PENDING the owner's x64 PC)

Fact:

- `packages/desktop/package.json`: new electron-builder `win` block — targets `nsis` + `dir`, arch `x64`, `icon: build/icon.png` (electron-builder generates the `.ico` from the 1024² png). New `nsis` block: `oneClick: false`, `perMachine: false`, `allowToChangeInstallationDirectory: true`. `extraResources` restructured — the shared migrations/persona/web stay at root; the sidecar is per-platform (`mac.extraResources` → `luna-server`, `win.extraResources` → `luna-server.exe`). Empirically confirmed electron-builder MERGES root + platform extraResources (the packaged mac app carries all four; the win-unpacked carries luna-server.exe + the shared three). New `pack:win` script (`electron-builder --win nsis dir`).
- `main.ts`: `app.requestSingleInstanceLock()` before `whenReady` (skipped under SMOKE); the losing instance quits, and a `second-instance` handler focuses the running window. `whenReady` guards on the lock. The `startWebHost` call passes an `onListenError` that shows an error dialog + quits (belt to the single-instance suspenders).
- `serve.ts`: `startWebHost` gains an `onListenError` param and attaches a `server.on('error', …)` so an EADDRINUSE (double-launch) surfaces to the caller instead of crashing the process. Electron-free (callback, not a dialog). +1 test (double-bind → EADDRINUSE fires the callback).
...
- The mac CAN cross-build the whole Windows deliverable — installer included — so the build loop stays on the fast machine and the x64 PC is purely for acceptance, exactly the Initiative's "build-from-mac, verify-on-PC" shape. (The NSIS step first failed on a truncated tool download mid network-switch; a retry with the toolchain cached completed — a flaky-network artifact, not a config error.)
- The single-instance lock + listen-error handler close the double-launch failure mode (common on the Windows taskbar) that would otherwise surface as an uncaught EADDRINUSE on the pinned 5177 host — but the *observable* behavior (focus the running window, no error) is one of the five real-machine checks, not something the mac smoke can prove.
- HONEST GAP: nothing here has run on real Windows. The five acceptance checks — installer runs, first boot reaches the wizard, a chat turn completes, double-launch focuses, quit leaves zero orphans (v0.38.0's tree-kill under real Windows), data persists across reboot — are the owner's to run on the x64 PC. The installer is the artifact for that.

### `v0.38.1` — 2026-07-17 — CI Windows matrix + portable web build (Initiative 28, 2/6)
...
- `.github/workflows/ci.yml`: the `test` job gains `strategy.matrix.os: [ubuntu-latest, windows-latest]` + `runs-on: ${{ matrix.os }}` + `fail-fast: false`. The `apt-get install ripgrep zsh` step is gated `if: runner.os == 'Linux'`; a `choco install ripgrep -y` step runs `if: runner.os == 'Windows'` (best-effort — the rg-using tests inject a fake runner). Two new steps run on both legs: **Build web (portable)** (`bun run --cwd packages/web build`) and **Cross-compile the Windows sidecar** (`compile:server:win`).
- NEW `packages/web/scripts/build.ts`: replaces the `build` script's `rm -rf dist && bun build … && cp … && mkdir -p … && cp -R …/. … 2>/dev/null || true` shell one-liner with node:fs (clean → `bun build` subprocess → stage assets). `package.json`'s `build` now runs it; the separate `build:assets` script is removed (folded in).
- NEW `packages/web/scripts/stageAssets.ts` + `.test.ts` (3 tests): `stageWebAssets(publicDir, dist, fs?)` — cubism core (required) + models tree (optional, preserving the old `|| true`), fs injectable for the test. build.ts calls it.
- Verified locally on the mac: the portable build produces byte-equivalent dist (cubism core + models staged); `compile:server:win` emits a real `PE32+ executable … for MS Windows` (100 MB) — the mac CAN cross-build the Windows sidecar. Suite 1420 green.

Inference:

- This is the regression net that makes the rest of Initiative 28 safe: every win32 platform branch from v0.38.0 (and every one to come) is now exercised by `bun test` on a real Windows runner each push, so path/spawn/`.exe` drift surfaces as a red check instead of a bug on the owner's PC. The unit tests assert the Windows *shape* from the mac; the matrix asserts the Windows *runtime*.
- The web build was the one remaining shell-shaped build step; moving it to node:fs removes the "does Bun Shell on Windows handle `cp -R …/.` and `2>/dev/null`" uncertainty entirely — the same script runs identically on every OS.
- HONEST GAP: the windows leg has not actually run yet. CI triggers on pushes to the public repo's `main` (via the publish flow), so the first real green/red windows result — and any Bun-on-Windows test surprises it surfaces (bun:sqlite, temp-path assumptions) — arrives with the next push, which is owner-gated. Written-and-locally-reasoned, not yet CI-proven on Windows.

### `v0.38.0` — 2026-07-17 — Windows process/path substrate (Initiative 28, 1/6)
...
- NEW `packages/desktop/src/childEnv.ts` (~35 lines) + `childEnv.test.ts` (8 tests): pure `serverBinName(platform)`, `pathKeyFor(platform, env)`, `childPathValue(platform, env, ffmpegDir)` — extracted from `main.ts` so the win32 env shape is testable off the electron-coupled boot path. `childPathValue` joins on the platform's own delimiter (`;` win32 / `:` else), prepends the ffmpeg dir, appends `/opt/homebrew/bin`+`/usr/local/bin` on darwin ONLY.
- `supervisor.ts`: `SpawnedChild.kill` gains an optional signal; new exported pure `killPlan(platform, pid)` → `{taskkill, args:['/pid',N,'/T','/F']}` on win32-with-pid else `{signal}`; new injectable `killFn` (default: taskkill tree-kill on win32; `child.kill()` SIGTERM + an unref'd `KILL_GRACE_MS` SIGKILL escalation on POSIX); `stop()`/`restart()` route the kill through it. `defaultSpawn` gains `windowsHide: true`. +5 tests (killPlan ×3, killFn routing ×2).
- `main.ts`: imports the childEnv helpers; `serverBin` resolves `serverBinName(process.platform)` (→ `luna-server.exe` on win32); the `createTtsSupervisor` PATH block replaced by `pathKeyFor` + `childPathValue` (removed the hardcoded `':'` join and the unconditional homebrew dirs).
- `ttsProvision.ts`: `windowsHide: true` on the `runProc` spawn + all three spawnSync probes (findFfmpeg/hasCompiler/findPython).
- `ttsRuntime.ts`: win32 bare-interpreter fallback is `python` (python.org), not `python3`. +1 test.
- `backend.ts`: `resolveDevLauncher` bun candidates honor `USERPROFILE` (win32 has no HOME) + a `bun.exe` path. +2 tests.
- `voicePack.ts`: `validateRuntimeDir(dir, platform)` + `startCommand(o, platform)` gain a win32 branch — `.venv\Scripts\python.exe` venv layout + a PowerShell-form copy-paste command. Both default `platform` to `process.platform` so the two `main.ts` callers are unchanged. +2 tests.
- `packages/desktop/package.json`: new `compile:server:win` script (`bun build --compile --target=bun-windows-x64 … --outfile bin/luna-server.exe`).

Inference:

- The orphaned-`luna-server.exe`-holding-the-SQLite-lock failure mode is the one that would corrupt the daily driver on Windows; landing the tree-kill seam FIRST means every later version (packaging, TTS, pet mode) inherits clean process teardown rather than each re-discovering the leak.
- Extracting the env logic into a pure module is the same "injectable seams" discipline the TTS provisioner used — it lets a mac CI leg assert the Windows string shape, which is exactly what v0.38.1's cross-platform matrix will lean on. Nothing here is verified on real Windows yet (that starts at v0.38.2); this is the unit-proven substrate.
- Deferred to later versions by design: the provisioned-checkout ffmpeg probe (v0.38.4 owns the bundled 整合包 layout), the electron-builder win target (v0.38.2), and the server-side shell/vec/secret gaps (v0.38.5).

### `v0.37.17` — 2026-07-16 — Location un-frozen: the auto cache no longer masquerades as a manual pin
...
- `packages/desktop/src/location.ts`: the persisted auto fix moves out of `LUNA_LAT_LON` into a new `LUNA_LAT_LON_AUTO` key. `resolveDesktopLocation` chain is now manual `LUNA_LAT_LON` → fresh CoreLocation (`persist: true`) → `LUNA_LAT_LON_AUTO` cache (new `'cached'` source, `persist: false`, backstop only) → timezone city. Added `coreLocationFixAsync` (same path ladder, `execFile`-based so a timer poll can't block Electron's main loop) and `movedBeyond` (`MOVE_EPSILON_DEG = 0.0005` ≈ 55 m — registers a real move, swallows Wi-Fi jitter).
- `packages/desktop/src/main.ts`: boot persist writes `LUNA_LAT_LON_AUTO` (the in-memory `userEnv['LUNA_LAT_LON']` sidecar injection is unchanged — weather still mounts at boot). New in-session watch: when no manual pin exists (darwin only), a 10-minute `setInterval` re-polls CoreLocation async; a move beyond epsilon updates the AUTO cache and broadcasts `luna:geo-fix` to every window.
- `packages/desktop/src/preload.ts`: `lunaPet.onGeoFix(cb)` — the shell's moved fix crosses into the renderer, which forwards it over its existing WS as `client.geo`.
- `packages/web/src/geo.ts`: `requestGeolocation` now uses `watchPosition` instead of a one-shot `getCurrentPosition` (fires on the initial fix and every real change; duplicate coordinates are dropped). New `setGeoFix` lets the desktop bridge update the cached fix so reconnect re-sends carry the newest location, not the page-load snapshot. `packages/web/src/app.ts` wires `bridge?.onGeoFix` → `setGeoFix` + `client.geo`.
- No server changes: `resolveLocation()` precedence (runtime GPS > env fallback) and the weather tool's call-time resolution were already correct — the bug was that no upstream ever sent a fresh coordinate after the first one.
- Owner-machine remediation: the polluted `LUNA_LAT_LON=34.373533,108.906581` line (machine-written by v0.33.0, masquerading as manual) is deleted from `luna.env`; the new code re-persists it as `LUNA_LAT_LON_AUTO` on next boot.
- +12 tests in `location.test.ts` (AUTO-cache chain ordering, manual-pin supremacy, `movedBeyond` jitter/move thresholds, async ladder). Suite 1399 green.

Inference:

- The freeze was v0.33.0's own cache poisoning its manual-pin escape hatch: persisting into the SAME key the operator uses made the code unable to tell "the human said so" from "I said so last time". Splitting the keys restores both invariants at once — the operator's word stays absolute, and the machine's word is always challengeable by a fresher fix.
- This regressed the product's most-liked location property (the owner noticed it tracked even small moves) precisely because the pre-persist behavior — re-acquire on every boot — was never a designed feature, just an accident of not caching. v0.37.17 makes the sensitivity deliberate: boot re-acquire + 10-min in-session re-poll on desktop, browser-announced movement on web, all flowing through the already-correct `client.geo` runtime channel.
- Closes the third leg of the v0.33.0 design ("a later CoreLocation grant auto-upgrades the timezone fallback") which the freeze had silently disabled: with the pin gone, a mid-session grant now upgrades within one poll cycle.

### `v0.37.16` — 2026-07-15 — No-compiler + GitHub-tarball time bomb (completed re-audit)
...
- The re-audit workflow finished this time (48/51 agents; the 3 losses were mid-stream connection
  resets while the owner switched networks). Its top confirmed blocker independently re-derived
  v0.37.15's Python-floor fix down to the exact regex change — validating that fix rather than adding
  work. Two findings survived past v0.37.15:
- **Compiler preflight (`ttsProvision.ts`)**: `jieba_fast` and `pyopenjtalk` are sdist-only on PyPI —
  0 wheels across every version (verified against the live index), so pip compiles them from source.
  A clean machine without Xcode CLT / build-essential passed python + ffmpeg preflight, downloaded
  ~1.6 GB, then failed at the venv stage. New `hasCompiler()` seam gated in preflight: `xcode-select
  -p` on macOS (exits 0 only with CLT, and unlike `cc` never triggers the install popup), `cc
  --version` on Linux; win32 skips (整合包 ships prebuilt). `COMPILER_HINT` names the per-OS remedy.
- **GitHub tarball checksum removed (`ttsProvision.ts`)**: the auto-generated source tarball was
  sha256-pinned, but GitHub does not guarantee byte-stability — its Jan-2023 gzip-level change flipped
  every such checksum and broke Homebrew formulae and Go's module cache. A pinned hash is a time bomb:
  on the next recompression every new install hard-fails at the first artifact. The pin is gone;
  integrity is structural (a wrong body — an HTML page — is not valid gzip, so `tar -xzf` fails at the
  extract stage), and a failed extract now `fs.remove`s the cached archive so a retry re-downloads
  instead of re-failing on the poisoned cache forever. All byte-stable artifacts (HF LFS, fasttext)
  keep their checksums.
- +4 tests. Suite 1384 → **1388 pass / 0 fail**; web + desktop + server tsc clean.

Inference:

- The compiler gap is the reference-machine pattern one more time: CLT is nearly universal on a dev
  Mac and absent on a fresh consumer Mac, and it is exactly what lets `pip install` a sdist-only
  package. Every "it built fine" was built by a compiler the target user may not have.
- The tarball pin is the opposite lesson from the cache-poisoning fix: a checksum is only an asset
  when the source is byte-stable. On a host that recompresses, the checksum stops being integrity and
  becomes a scheduled outage — so the right move is to drop it there and enforce integrity by whether
  the thing actually extracts, with a cache that clears itself on failure.

### `v0.37.15` — 2026-07-15 — Re-audit fixes (Python floor, torch ceilings, live pack-swap, honest badge, no orphans)
...
- The re-run audit (6 lenses, 3 verifiers/finding; the first attempt died to the owner's network
  switching, this one completed recon+verify) produced 16 findings / 26 verdicts. Five landed:
- **Python floor → 3.10** (`ttsProvision.ts`): a clean Mac with only `/usr/bin/python3` (3.9.6, the
  common case after `brew install ffmpeg` without `python@3.11`) built a 3.9 venv. Verified on PyPI:
  torch/transformers/tokenizers/numba all require ≥3.10, so 3.9 silently resolves *downgraded* builds
  (a real test landed torch 2.8 + numpy 2.0) that were never validated against the 2025 code tag.
  `PYTHON_CANDIDATES` drops all 3.9 entries and bare `python3`; the version regex is `/^3\.(10|11)$/`.
- **torch trio ceilinged** (`ttsProvision.ts`): `torch`/`torchaudio`/`torchcodec` were unpinned, so the
  next major would land on every fresh install with nothing to stop it. Now `torch>=2.4,<2.14` (etc.),
  which a real resolve confirmed still lands the proven 2.13/2.11/0.15. torchcodec moved out of
  `INFERENCE_REQUIREMENTS` into the torch install line.
- **In-app pack drop switches the live voice** (`main.ts`): the managed install branch wrote the yaml
  and started api_v2 but never set `LUNA_TTS_BACKEND=http` or reloaded — the renderer stayed on the
  browser sink, so the cloned voice ran *unused* while packDrop reported "✓ Voice swapped". Now writes
  the backend and reloads non-setup windows (shared `reloadAppWindows`, mirroring the model install).
- **Honest health badge** (`setupWizard.ts`): `up = res.ok` was green for the managed 200s
  `{state:'starting'|'gave-up'}`, so it showed ready during the ~13 s load and stayed green on a
  crash-looped child. Reads the body now (green only on real `ready`; a distinct "warming" for
  starting/restarting; down on gave-up).
- **No orphaned installers** (`ttsProvision.ts` + `main.ts`): the pip/torch children (10-30 min) were
  orphaned on quit and their half-built venv collided with the next resume. Tracked in `liveProcs`;
  `killProvisioners()` runs on both quit paths beside the supervisors.
- Verdict-refuted findings (recorded, not acted on): the "no C compiler" blocker was downgraded to
  major by verification (most 3.11 install paths carry a toolchain); several dep-reproducibility notes
  were judged minor. +6 tests. Suite 1381 → **1384 pass / 0 fail**; web + desktop tsc clean.

Inference:

- The 3.9 finding is the same shape as every earlier TTS bug and the reason the "it synthesized"
  proof kept being premature: the reference machine had python 3.11, so the 3.9 path was invisible
  until an agent built a venv with `/usr/bin/python3`. The fix is a floor, not a fake-out.
- Known limitation, left as-is with a note: `LUNA_TTS_HF_MIRROR` rewrites only the HuggingFace host,
  so a CN user behind the GFW still dead-ends on the GitHub tarball, the nltk zips, and lid.176.bin.
  A general proxy override is a real feature, not a one-line fix — deferred, but the changelog and the
  hint should stop implying the mirror covers everything.

### `v0.37.14` — 2026-07-15 — Audit hardening (mirror-safe checksums, weight picker, ffmpeg memoized)
...
- **CN mirror × the new sha256 gate — confirmed safe, not a regression.** `hf-mirror.com` was checked
  to serve byte-identical content: its `chinese-roberta-wwm-ext-large/config.json` hashes to the exact
  `3d57de2f…` the manifest pins (and the LFS etag for the 651 MB bin matches the value hashed by hand).
  So a `LUNA_TTS_HF_MIRROR` user is NOT hard-failed by the checksum. No code change — a verification.
- **`adoptInstalledPacks` weight pick, `main.ts`:** was `firstFileIn` = first non-dotfile, so a
  `readme.txt` (or a `._`-fork) alphabetically before the weight would be written into the yaml as the
  model. Replaced with `pickWeight(names, ext)` (moved to `ttsRuntime.ts`, electron-free): match the
  `.ckpt`/`.pth` extension, skip dotfiles and AppleDouble forks. +4 tests in `ttsRuntime.test.ts`.
- **`findFfmpeg()` memoized (`main.ts`):** it spawns `ffmpeg -version`, and a voice swap re-creates the
  supervisor, so it re-spawned on the UI thread each time. `cachedFfmpeg()` probes once per session.

Inference:

- The re-audit (6 lenses, 3 verifiers each) kept dying to connection resets mid-run — the owner was
  switching networks. Rather than block on it, its three landed findings were verified against the
  live hosts and code and closed. The mirror-safety check is the important one: the v0.37.12 checksum
  fix could have silently broken the exact CN path `LUNA_TTS_HF_MIRROR` exists for, and it does not.
- These are hardening, not blockers: the pack-weight case needs a malformed pack dir, and the ffmpeg
  spawn was correct if slightly wasteful. The re-audit is still running for the lifecycle / UX /
  offline-synth lenses the first pass never reached.

### `v0.37.13` — 2026-07-14 — The clean room wasn't clean: English was broken for every new user
...
- v0.37.12 closed with "verified: api_v2 boots and synthesizes 5.64 s of real audio". That run was on
  the reference machine, whose `$HOME` has held `~/nltk_data` since **June 8** and pyopenjtalk's
  cached dictionary. Re-running the same synth under a **fake `$HOME`** — the only way to simulate a
  machine that has never run GPT-SoVITS — produced what a real new user gets:
  - `g2p_en` → `LookupError: Resource 'cmudict' not found`. nltk does **not** auto-download it; it
    raises. **English synthesis dies on the first utterance.** Luna speaks English.
  - `pyopenjtalk` → its `open_jtalk_dic_utf_8-1.11` is absent; it fetches 22.6 MB from GitHub on
    first use — i.e. mid-conversation, over the network, or not at all.
- **The corpora are manifest artifacts now**, not an `nltk.download()` call at install time:
  `nltk/cmudict`, `nltk/averaged_perceptron_tagger_eng`, `nltk/averaged_perceptron_tagger`
  (checksummed, resumable, mirrorable, counted in the progress bar, and the voice then works
  offline). `NLTK_DATA` is set on the api_v2 child — nltk searches `$HOME` and the venv, never the
  checkout, so putting the corpora in the runtime is not enough by itself. pyopenjtalk's dictionary
  is force-fetched during the install. `validating` now fails if the English corpus is missing.
- **All 12 artifacts carry a sha256.** The Windows 整合包's (8.19 GB, never downloaded here) comes
  from HuggingFace's LFS etag — validated by checking HF's etag for a file I *had* hashed by hand
  (roberta's 651 MB bin) and finding it identical.
- **Verified**: under a fake `$HOME` **and** a minimal PATH (`/usr/bin:/bin` + homebrew), api_v2 boots
  and `POST /tts` returns real speech — English 4.84 s / RMS 4198, mixed 中英 3.60 s / RMS 3793.
  Language segmentation splits 中/英 correctly. Suite 1374 → **1377 pass / 0 fail**.

Inference:

- The reference machine has now hidden a bug at **every single layer**: it had python 3.11 (hiding the
  hardcoded `python3`), ffmpeg (hiding the system dependency), a *newer* GPT-SoVITS checkout (hiding
  the code-tag incompatibility), a full shell PATH (hiding the packaged-app PATH), and — this time —
  language corpora in `$HOME` from a month ago. Each layer of "it works on the machine the recipe came
  from" was a layer of the recipe being wrong.
- The methodological correction is small and permanent: **an end-to-end run is only as clean as its
  `$HOME`, its `PATH`, and its network.** v0.37.12 controlled the directory and called it a clean
  room. It wasn't one. Fake the environment, not just the folder.

### `v0.37.12` — 2026-07-14 — The one-click voice install, proven by actually running it (owner-demanded)
...
- The owner refused to accept "every URL HEAD-checked" as proof of a working install. A real
  clean-room run was performed: an isolated directory, `buildManifest` + `runProvision` + `realSeams`
  driven end to end (**1.57 GB downloaded**), then `api_v2.py` launched from the provisioned runtime
  with `generateTtsYaml`'s own output, then a synth issued through `planTtsForward`'s own payload.
- **Six blockers, none of which any amount of code review would have found:**
  1. **`python3` hardcoded** (`exec('python3', ['-m','venv'])`). The reference machine's `python3` is
     **3.14** — no torch wheels. Preflight checked disk but not python, so the failure landed AFTER a
     ~1.6 GB download. Fix: `findPython()` seam (probes `python3.11/3.10/3.9`, asks each its own
     version), gated in **preflight**.
  2. **The upstream `requirements.txt` cannot be installed on python 3.11.** It pins
     `numba==0.56.4`, whose own installer says: *"Cannot install on Python version 3.11.15; only
     versions >=3.7,<3.11 are supported."* GPT-SoVITS's recommended python and its own requirements
     file are mutually exclusive. Fix: Luna ships `INFERENCE_REQUIREMENTS` — a curated list derived
     from api_v2's real import graph, with torch installed separately (CPU index on Linux, since
     PyPI's linux torch is a ~2.5 GB CUDA build).
  3. **The pinned code tag crashes on import with modern torch.** `20240821v2`'s
     `AR/modules/patched_mha_with_cache.py:41` annotates with `Tuple`/`Tensor` names it expects
     `torch.nn.functional` to re-export; torch stopped. → `NameError: name 'Tuple' is not defined`.
     The reference instance runs **`20250606v2pro`** — identified by sha256-matching that very file
     against every release tag and by the presence of `GPT_SoVITS/sv.py` (v2Pro-only, and in its
     import graph). `CODE_TAG` repinned; the Windows 整合包 moved to the matching `v2pro-20250604`
     build so both platforms run one code generation.
  4. **Three missing deps**, each found by a boot that died at it: `matplotlib` (imported at module
     scope by `AR/modules/lr_schedulers.py`), `split-lang` (`text/LangSegmenter`), and **`gradio`** —
     which looks like pure WebUI weight but is load-bearing: `tools/my_utils.py` imports it and
     `TTS.py` imports `tools`. My guess had explicitly excluded it.
  5. **A dependency conflict that only appears when both are installed**: gradio's latest drags in
     `huggingface-hub` 1.x, which transformers rejects at import ("*huggingface-hub>=0.34.0,<1.0 is
     required … but found 1.23.0*"). Pinned `huggingface-hub>=0.26,<1`.
  6. **`torchcodec` + the system `ffmpeg` binary.** torchaudio ≥2.9 decodes through torchcodec, which
     shells out to ffmpeg. Without them the install *succeeds* and then **every synth 400s** with
     "TorchCodec is required for load_with_torchcodec". The reference machine has ffmpeg via Homebrew,
     which is exactly why it was invisible. `torchcodec` added; `hasFfmpeg()` seam gates **preflight**.
- **Proof of life** (this is the first version of the TTS init that has any): `api_v2` boots from the
  provisioned runtime — its startup log is line-for-line the reference instance's (same
  `_IncompatibleKeys(enc_q…)` notice, BERT + CNHuBERT loaded from the **provisioned** dirs) — and a
  `POST /tts` returns **HTTP 200, audio/wav, 5.64 s, 32 kHz, RMS 4151, peak 21376**: real speech, not
  silence. Resume was exercised for real too (9 completed artifacts skipped in 0 s on a re-run).
- **A 6-lens adversarial audit ran alongside the e2e** (each finding refuted by two independent
  verifiers; network flakiness killed 5 of the 6 recon lenses, so this is PARTIAL coverage — the
  state-machine lens alone produced 13 findings, 6 confirmed). It found five things the end-to-end
  run could not, because the run had a full shell PATH and a healthy network:
  - **The packaged app's `python3` is not the user's python3.** A Finder-launched `.app` inherits
    `PATH=/usr/bin:/bin:/usr/sbin:/sbin` — Homebrew's `/opt/homebrew/bin` is NOT on it. Probing bare
    names would ENOENT on a machine that HAS python 3.11 and fall through to the system 3.9.6.
    Verified: `env -i PATH=/usr/bin:/bin sh -c 'command -v python3.11'` → not found. `PYTHON_CANDIDATES`
    now probes ABSOLUTE locations first (the same pattern `resolveDevLauncher` already used for bun),
    and the same for ffmpeg. **The api_v2 CHILD needs ffmpeg on its PATH too** (torchcodec shells out
    to it), so the supervisor now prepends the discovered ffmpeg dir to the child's PATH — otherwise
    the voice installs perfectly and then 400s on every utterance.
  - **Cache poisoning, proven by driving the real `runProvision`.** HuggingFace GZIPS the small JSONs,
    so `content-length` is ABSENT → `served === 0` → the only integrity check silently no-ops. A
    captive portal's 200 HTML page lands as `tokenizer.json`, is committed, and — since an existing
    file was never re-verified — is reused forever; `validating` (which only stat'd two directory
    NAMES) passes, the marker says `ready`, and api_v2 crash-loops on a parse error with no UI path to
    recovery. Fixed: **sha256 on every artifact** (measured from the clean-room download that then
    successfully ran), verified before the rename AND on every cached file; a mismatch deletes and
    re-fetches. The cache key now includes the URL — v0.37.9 moved G2PWModel off a DEAD host under an
    unchanged name, so a name-only key would have silently reused the broken file.
  - **A pack dropped before the runtime exists is lost forever.** The drop-zone sits right next to the
    deploy button. Installing a pack while the runtime is still downloading wrote the weights but no
    yaml (there was no checkout to point at) — and nothing ever wrote it afterwards, so the voice would
    never start, silently. `adoptInstalledPacks()` now runs when provisioning completes.
  - **Linux could never extract G2PW**: the zip was handed to `tar`, and GNU tar — which IS `tar` on
    Linux — cannot read zip at all (bsdtar, on macOS/Windows, can, which is why it looked fine). Linux
    uses `unzip` now.
  - **pip's failure output was thrown away** (stdout `ignore`d, stderr truncated to 300 chars) — the
    most likely failure of the whole flow surfaced as an unactionable stub. Both streams are captured
    now, and the TAIL (where pip's real error lives) is reported.
  - Also fixed: a `.part` at or past the remote size can never complete (a Range beyond EOF is a
    permanent 416) → it restarts instead; `validating` checks every load-bearing piece (the model
    FILES, G2PW, the venv), not two directory names; the disk floor (10 GB) was smaller than what the
    install actually needs → 12 GB scripted / 25 GB Windows.
- **The browser voice** (`webSpeechSink.ts`): the utterance carried no `voice` and no `lang`, so the
  zero-setup path spoke in the **OS default** — on macOS/EN a deep male voice, behind a girl's avatar.
  New `voicePick.ts`: language-matched (Han→zh-CN, kana→ja-JP), female-preferred, known-male voices
  **never** eligible, `localStorage['luna:voice']` overrides. `getVoices()` is async and returns []
  on first call, so the sink now waits (bounded 1.2 s) for `voiceschanged` — the first utterance was
  the most likely one to come out wrong. Its own first draft fell into the substring trap (`'man'`
  matches "Sa**man**tha", `'male'` matches "Fe**male**"); matching is word-boundary now, with tests
  pinning both traps.
- **Tests** (+14): python gate (no interpreter → preflight fail, zero downloads), ffmpeg gate,
  discovered-interpreter-is-the-one-used, CPU torch index on Linux, the dep list's exclusions AND its
  surprising inclusions, voicePick (never a man, the two substring traps, async-empty list).
  Suite 1346 → **1374 pass / 0 fail**; all four packages' tsc clean.

Inference:

- Every previous "the voice install works" claim in this changelog was false, and each was made in
  good faith on the strength of a check that never executed the thing it certified: v0.37.2 shipped
  it unrun; v0.37.9 HEAD-checked the URLs; v0.37.10 fixed a 404 the HEAD-check had skipped. The
  install failed at **six** more points past the last URL. The only check that found them was
  running it.
- The reference machine was the worst possible witness: it already had python 3.11, ffmpeg, a working
  venv, and a checkout of a *different, newer* GPT-SoVITS than the one the manifest pinned. Every one
  of those made a blocker invisible. A "works on my machine" that is itself the source of the recipe
  is not evidence — it is the way the recipe gets its bugs.
- The tightest lesson of all six: **guessing the dependency list from a requirements file got it
  backwards in both directions.** It excluded gradio (load-bearing) and included funasr/modelscope
  (never imported). The import graph and a real boot were the only sources of truth.

### `v0.37.11` — 2026-07-14 — CI red on every push: the runner has no zsh (owner-asked)
...
- Every push to the public repo failed CI's `Test` step, from the FIRST public run onward. Failing
  pair, both in `shellCore.test.ts`, both dead in <1ms: the zsh-`command`-path `$()`-interpretation
  proof and the process-tree kill — the real spawner execs `/bin/zsh -lc` (`shellCore.ts:90`) and
  ubuntu-latest ships no zsh → spawn ENOENT.
- **`.github/workflows/ci.yml`**: the runner now installs `zsh` next to ripgrep (the two invariants
  actually execute on Linux instead of being skipped), and gains a `Type-check (desktop)` step — the
  workflow predates `packages/desktop`, so 30+ desktop sources (all of Initiative 27) were never
  type-checked in CI.
- **`shellCore.test.ts`**: `hasZsh = existsSync('/bin/zsh')`; the zsh-path test and the kill describe
  are `skipIf(!hasZsh)` — a zsh-less machine skips them (the argv-path tests still run everywhere).
- Suite **1346 pass / 0 fail** locally (zsh present → nothing skipped); server tsc clean.

Inference:

- The failure mode rhymes with v0.37.9/v0.37.10: an invariant that only ever executed on the owner's
  mac (zsh preinstalled) read as green forever, and the one environment without the dependency — the
  CI runner, then any Linux user — hit it immediately. Same lesson, third costume: coverage that
  never runs in the environment it protects is not coverage.
- Recorded as a known gap, deliberately NOT changed here: the shell TOOL hard-requires `/bin/zsh` at
  runtime, so a zsh-less Linux install has a broken `shell` tool today. Falling back to bash would
  silently change shell semantics; that is its own decision, not a rider on a CI fix.
...
- **`main.ts`**: `luna:provision-start` no longer requires `LUNA_TTS_PROVISION=1`; the flag is now an
  opt-OUT (`=0`). `envfile.ts` template + `SETUP.md` + the wizard hint updated accordingly.
- **Validation that should have run before v0.37.2 shipped**: every manifest URL HEAD-checked against
  the real hosts (2026-07-14). Results: GitHub code archive 200 (chunked, no content-length); roberta
  `pytorch_model.bin` 200 but **651,225,145 bytes**, not the 1,306,484,351 I had hardcoded; hubert
  200 / 188,811,417 (matched); lid.176 200 / 131,266,198; Windows 整合包 200 / 5,744,891,255; HF
  serves `206` for a Range request (resume works); **G2PWModel bcebos URL: connection failure**.
- **Three fixes**: (1) G2PW → `XXXXRT/GPT-SoVITS-Pretrained/resolve/main/G2PWModel.zip` (the URL
  GPT-SoVITS' own docs use) — an HF URL, so `LUNA_TTS_HF_MIRROR` covers it; its zip's top level IS
  `G2PWModel/`, so `dest` becomes `GPT_SoVITS/text` with no `stripPrefix`. (2) Completion is verified
  against the **server's content-length** for that transfer (`served`), never a hardcoded constant;
  `sizeBytes` is demoted to a progress hint (real measured values). A file at its final path is
  trusted — the `.part` → final rename IS the commit. (3) Windows: the 7-Zip-missing error now tells
  the user to install it and retry (the download is already saved, so retry resumes instantly).
- **Tests**: the fake download seam's `total` contract corrected (it had reported REMAINING bytes;
  `realSeams` reports the full final size — the fake had been hiding the truncation path); +2 tests
  (truncated transfer fails loudly and never commits; a file at its final path is trusted), 3
  rewritten (mirror now covers G2PW; the dead host is asserted gone; size hints are 0-or-real).
  Suite 1343 → **1345 pass / 0 fail**; web + desktop tsc clean.

Inference:

- The owner's question ("why does a one-click install need me to flip a flag in env?") was the right
  one twice over. It was a real product bug — a one-click flow gated behind a hand-edited config file
  is the terminal step this initiative exists to delete, wearing a disguise — and it was **hiding a
  dead feature**: with the bcebos URL dead and the roberta size wrong, the button could not have
  succeeded on any machine, ever. Shipping it default-off is exactly what let both bugs survive code
  review, tests, and a packaged smoke: nothing in the loop ever made a real request.
- The lesson generalizes past this version: a flag that no one flips does not de-risk a feature, it
  **conceals** it (the same shape as the skills system that sat inert for versions). The bar for
  "unvalidated" should be "the button is hidden AND the network path is exercised in CI", not "the
  user must find a flag".

### `v0.37.8` — 2026-07-14 — Re-running setup preserves the configuration (owner-caught)
...
- **`onboarding.ts`**: new `SECRET_KEYS` (the four API-key envs) + pure `wizardPrefill(env)` →
  `{values, configured}` — non-secret `WIZARD_KEYS` come back as values, secret keys as NAMES only.
- **`main.ts`**: `luna:wizard-prefill` handler (reads `luna.env` fresh); **`preload.ts`** exposes
  `wizardPrefill()`.
- **`setupWizard.ts`**: pure `hydrateWizardValues(saved, configured, specs)` — a saved value beats a
  spec's static `initial`, and a configured secret sets no value at all. `fieldRow` gained a
  `configuredHint` (a stored secret renders "already set · leave blank to keep" over an EMPTY
  input). `mountSetupWizard` fetches the prefill before the first paint and hydrates `values`,
  `configuredSecrets` and `voiceBackend` (from a saved `LUNA_TTS_BACKEND`).
- **Tests** (+8): the clobber case (a saved gateway URL / model survives a re-run), a fresh install
  still gets the stock defaults, a secret is a name-not-a-value (the payload is asserted not to
  contain the key text), empty fields are never collected, hydrate→collect round-trip.
  Suite 1335 → **1343 pass / 0 fail**; web + desktop tsc clean.

Inference:

- This was not the cosmetic "the wizard forgot my values" it looked like — it was **config
  destruction**. A field with an `initial` wrote that default into the submit map at render time, so
  a user who re-ran setup to change one thing silently had their custom `ANTHROPIC_BASE_URL` and
  `LUNA_MODEL` overwritten and their GPT-SoVITS voice reset to the browser voice. The bug shipped
  with the wizard in v0.35.0 and survived four initiatives because the wizard had only ever been
  exercised as a FIRST run.
- The prefill keeps the v0.28.0 key-custody rule intact: values flow renderer → shell and never
  back. Telling the renderer only that a key EXISTS gives the user the "my config is still here"
  signal with no secret in the page, and the empty-field-drops-at-submit path (true since v0.35.0)
  is what makes "leave blank to keep" work with no new preservation logic at all.

### `v0.37.7` — 2026-07-14 — They, not he
...
- `docs/history/DEVELOPMENT.md`: three passages in the v0.37.0/v0.37.2/v0.37.3 records switched from
  he/his to they/them for the repo owner.

Inference:

- Pre-publish hygiene, caught while gating the push: the owner's pronouns were never stated, and a
  public changelog should not guess. Luna's own she/her is unchanged — that is the character.

### `v0.37.6` — 2026-07-14 — README voice copy synced to the one-click reality
...
- `README.md` + `README.zh-CN.md`: the 🗣 voice feature bullet and the wizard-voice screenshot
  caption rewritten around the v0.37.x flow; `docs/assets/wizard-voice.png` replaced with the
  isolated-userData smoke capture of the NEW voice step (one-click deploy button + rewritten guide).

Inference:

- The wizard's in-app guidance was updated in v0.37.2/v0.37.4, but the public front pages still
  promised a copy-paste launch command — the exact thing this initiative removed. Owner-caught
  ("对应的前端引导也改过了没"); lesson: a flow change's copy sweep must include the READMEs and
  their screenshot assets, not just the in-app strings.

### `v0.37.5` — 2026-07-14 — Smoke web-port isolation (found by the first live-instance-parallel smoke)
...
- `main.ts`: new `DESKTOP_WEB_PORT = Number(LUNA_DESKTOP_WEB_PORT ?? (SMOKE ? 5178 : WEB_PORT))`;
  the three `WEB_PORT` call sites (window URLs + `startWebHost`) now use it.

Inference:

- Every earlier smoke happened to run while the packaged app was closed, so the pinned 5177 never
  collided. Today's end-of-initiative smoke ran with the owner's app open → `startWebHost` hit
  EADDRINUSE and the run wedged before the probe with zero output (the launcher had also filtered
  non-JSON stdout — a second lesson: never grep away a smoke's logs). The fix mirrors the WS port's
  existing smoke isolation; a verification run can now never collide with, or attach to, a live
  instance through the web host either.

### `v0.37.4` — 2026-07-14 — 兜底阶梯 + 首句保护 + 文案收口 (Initiative 27, 5/5 — closes the initiative)
...
- **`webAudioSink.ts`**: `WebAudioSinkOpts` gains `onUnspoken(text, voice)` + an injectable
  `fetchSpeechFn`. `fetch` now returns `ArrayBuffer | 'failed' | null` (null = barge-in abort only);
  the serialized queue runner hands a `'failed'` utterance to `onUnspoken` (so the fallback never
  overlaps http playback), and the 60s mute window routes utterances straight to the fallback rung
  instead of dropping them. `app.ts` wires `onUnspoken` to a lazily-created `WebSpeechSink` with a
  logged reason — the next utterance retries http, so recovery re-promotes by itself.
- **Copy**: `guide.voice` rewritten (one-click deploy → drag pack → badge green; drop-anywhere swap;
  no terminal), the runtime picker relabeled "Advanced" with an honest "(optional — skip if you used
  one-click deploy)" hint; `docs/SETUP.md` voice section + the guided-way paragraph rebuilt around
  the managed flow, with docker/BYO demoted to an advanced appendix and the loopback-only spawn rule
  stated.
- **Tests**: NEW `webAudioSink.test.ts` (4, injected fetch + rAF stubs, failure-side paths only —
  success playback needs AudioContext): hard failure → unspoken; barge-in abort → NOT unspoken;
  mute window → straight to fallback with no http attempt; 502s → per-utterance fallback with no
  latch accrual. Suite 1331 → **1335 pass / 0 fail**; web + desktop tsc clean.
- Scoped decision recorded: the serve-side "hold /speak until the managed child binds" idea was
  dropped — the ladder makes a mid-swap utterance SPEAK (browser voice) instead of making the user
  wait on a held request; boot-time waiting already belongs to the v0.37.1 gate.

Inference:

- The ladder closes the last silent-failure hole in the voice path: before this, `fetch → null`
  meant a sentence simply vanished (the exact class of bug as the ghost-voice incident's silent
  double-speak, inverted). Now every utterance is spoken by SOME rung or was deliberately barged-in.
- **Initiative 27 is code-complete**: 托管骨架 (v0.37.0) → 冷启动加载页/标准 2 (v0.37.1) → 一键部署/
  标准 1 (v0.37.2) → 拖包换音色 (v0.37.3) → 兜底阶梯 (v0.37.4). The one honest gap: the per-OS
  REAL-download validation of the installer hasn't run (GB-scale, needs a clean machine per OS), so
  `LUNA_TTS_PROVISION` ships default-off; flipping it is a config change once validated, not a code
  change.

### `v0.37.3` — 2026-07-14 — 拖包换音色: a dropped pack hot-swaps the managed voice (Initiative 27, 4/5)
...
- **`ttsRuntime.ts`**: `resolveManagedCheckout(env, {userData})` extracted — the launchable checkout
  WITHOUT the pack-yaml requirement, i.e. what a FIRST pack install writes its yaml against
  (`resolveManagedRuntime` now composes it). +2 direct tests.
- **`main.ts`**: the `luna:install-voice-pack` handler branches on `LUNA_TTS_MANAGED=1` — managed:
  write `tts_infer.runtime.yaml` into the pack dir against the managed checkout, re-resolve the full
  runtime (the new yaml is now newest), `swapManagedTts(rt)`, return `{ok, refAudio, managed, ready}`
  — no `command`, no `runtimeDir` requirement. `swapManagedTts`: stop the old child → an api_v2
  STILL listening is external (the owner's own instance) → adopt, never restart it → else a fresh
  supervisor on the new argv (argv is fixed at creation, so swap = recreate, via the extracted
  `createTtsSupervisor`) → bounded 60s port wait (the wizard badge keeps polling either way). SMOKE
  no-ops. Unmanaged keeps the legacy command path byte-identical.
- **NEW `packages/web/src/ui/packDrop.ts`** + app.ts mount (desktop-only, gated on the lunaSetup
  scan/install bridges): drag a pack folder anywhere onto the RUNNING app → scan → a confirm chip
  (应用/取消) → install → the swap result flashes (已切换 ✓ / 就绪后自动应用 / BYO 手动重启). Pure
  `autoPicksFrom` (exactly-one-of-each auto-picks; ambiguity → wizard) + `swapResultText`; chip CSS
  in `theme.css`. `packDrop.test.ts` (8).
- Suite 1321 → **1331 pass / 0 fail**; web + desktop tsc clean.
...
- **NEW `packages/desktop/src/ttsProvision.ts`** (~380 lines): `buildManifest` (per-OS artifact
  list) + `runProvision` (the stage machine: preflight → downloading → extracting → materializing →
  venv → validating → ready, persisted to `userData/tts/provision.json` after every transition) +
  `realSeams` (node fetch with Range resume into `.part` files; system extractors via vetted argv —
  tar/ditto, 7z with a top-folder hoist and a clear install-7-Zip-or-manual error; venv/pip exec;
...
- **The mac manifest is the owner's verified lean recipe** (grounded on their working install): code
  ...
  `GPT-SoVITS-v2-240821.7z` (5.74 GB) with `stripPrefix` hoisting. `LUNA_TTS_HF_MIRROR` swaps the
  HF host (hf-mirror) for CN networks; non-HF origins untouched.
- **`ttsRuntime.ts` correction (Open Q4 landed)**: NO installed pack → `resolveManagedRuntime`
  returns null — GPT-SoVITS is zero-shot, a voiceless launch would crash-loop on the lean runtime's
  deliberately-absent stock checkpoints; the app keeps the browser voice until the first pack drops.
  Also the 整合包's embedded `runtime\python.exe` resolves on win32 when no venv exists.
- **Wiring**: `luna:provision-start` (kick/resume; guarded on the `LUNA_TTS_PROVISION=1` preview
  flag; returns immediately) + `luna:provision-status` (snapshot + `inFlight`, hydrated from the
  marker so a quit mid-install reads as paused) in `main.ts`; on `ready` the handler arms
  `LUNA_TTS_MANAGED=1` (backend stays browser until a pack lands). `preload.ts` exposes both.
  Wizard voice step: a 一键下载并部署 button + polled staged status line (`provisionCopyKey` pure
  mapping); bilingual strings in `setupCopy.ts` incl. the honest hint (~2 GB / ~5.7 GB, resumable,
  drop-a-pack-afterwards). `LUNA_TTS_PROVISION` + `LUNA_TTS_HF_MIRROR` in ENV_TEMPLATE + WIZARD_KEYS.
- **Tests** (+15): `ttsProvision.test.ts` (9 — stage order incl. win32 no-venv, preflight fast-fail,
  skip/resume/mismatch-demote, fail→persisted marker→rerun-completes-remainder, ready short-circuit,
  manifest sanity + mirror override) · ttsRuntime updated (pack-required, embedded win python) ·
  `provisionCopyKey` mapping + zh/en key-existence. Suite 1306 → **1321 pass / 0 fail**; web +
  desktop tsc clean.

Inference:

- 标准 1's shape is now real end-to-end on the code path: wizard button → resumable download →
  runtime materialized → managed mode armed → (v0.37.3) the first pack drop starts the voice. The
  flag stays off by default because the honest validation bar — a REAL multi-GB download on each OS —
  hasn't run yet; everything below that bar (stage machine, resume, marker, wiring, copy) is
  test-pinned.
- Grounding the manifest on the owner's working install paid off twice: the download shrank from a
  feared ~5 GB to ~2 GB (no gsv checkpoints), and it surfaced the no-pack/no-launch correction that
  saved the supervisor from a guaranteed crash-loop on fresh installs.

### `v0.37.1` — 2026-07-14 — 冷启动加载页: the gate learns the managed lifecycle (Initiative 27, 2/5 — 标准 2)
...
- **`bootGate.ts` / `warmUpTts`**: the warm-synth racer becomes a retry loop — while the last-seen
  health state is `starting`/`restarting` (v0.37.0's managed signal), a refused/failed `/speak`
  schedules a retry instead of `finish('failed')`; unmanaged failures keep the one-shot fast-fail.
  `gave-up` fails the gate immediately, both on the FIRST health read and mid-poll. `restarting`
  added to `TTS_STATE_LABEL`. `onStatus(text, state?)` now surfaces the raw state; all timings
  (`pollMs`/`deadlineMs`/`synthRetryMs`/`synthTimeoutMs`) injectable for tests. `BootGate` gains
  `setSkipHidden`.
- **`app.ts`**: on the first `starting`/`restarting` status the gate hides Skip and reveals it at
  20s-from-gate-start; warm-up resolution (ready/failed/unavailable) reveals it unconditionally.
- **Tests**: NEW `warmUpTts.test.ts` (7, scripted global fetch + fast timings): managed wait retries
  synths until health-ready; gave-up fast-fails (first + mid-poll, measured under the deadline); BYO
  502 → 'unavailable' unchanged; already-warm no-synth; unmanaged synth fast-fail preserved; the
  deadline still lifts a never-completing managed start. Suite 1299 → **1306 pass / 0 fail**;
  web + desktop tsc clean.
- Scoped OUT (recorded): the serve-side `/speak`-holds-until-bound wait — the web-side synth retry
  covers the boot gate; holding real user speaks across a restart is v0.37.4's first-utterance
  protection.

Inference:

- This is the whole of 标准 2's semantic switch, landed in ~40 lines because v0.37.0 put the state
  in the health answer and the gate already existed: "冷启动完毕(加载页)后再进入 app" now holds
  for a managed voice, while every BYO/browser/voiceless path is pinned byte-identical by tests.
- The synth-retry was the hidden second half of the bug: fixing only the health poll would still
  have failed the gate via the racer's first refused `/speak`. The retry is gated on the managed
  states precisely so BYO keeps its honest fast-fail.

### `v0.37.0` — 2026-07-14 — Managed TTS supervisor seam (Initiative 27 「自动语音」, 1/5)
...
- **NEW `packages/desktop/src/ttsRuntime.ts`** (~140 lines, Electron-free, injectable fs):
  `parseLoopbackUrl` (manages ONLY 127.0.0.1/localhost — a remote `LUNA_TTS_URL` is never ours to
  spawn), `resolveManagedRuntime(env, {userData})` (gated on `LUNA_TTS_MANAGED==='1'`; precedence:
  wizard-provisioned runtime with `provision.json state==='ready'` → BYO `LUNA_TTS_RUNTIME_DIR`
  checkout with api_v2.py + both pretrained dirs; voice = newest installed pack's
  `tts_infer.runtime.yaml`, else the checkout's stock config; venv python per-platform else
  `python3`), and `buildTtsArgv` — the reference-instance launch (`api_v2.py -a -p -c`) in **argv**
  form for the supervisor, never `startCommand`'s shell string.
- **`main.ts`**: a third `createSupervisor` (`ttsSupervisor`) started via `maybeStartManagedTts` in
  every APP mode (attach/dev/sidecar), never in setup mode, never under SMOKE; an api_v2 already on
  the port is **adopted** (`waitForPort` probe), never doubled; supervisor events map to a
  `ttsProcState` (`starting/restarting/gave-up`); both `before-quit` AND `window-all-closed` now stop
  it (no orphaned 9880).
- **`serve.ts`**: `startWebHost` gains an optional `voiceState` getter; the `/api/tts/health` forward
  answers `200 {backend:{ready:false,state:'starting'|'restarting'|'gave-up'}}` when the upstream is
  unreachable but the managed child is in flight, and keeps the bare 502 when unmanaged. A reachable
  upstream still reports ready regardless (api_v2 loads models BEFORE binding — the corrected fact).
- **Config**: `LUNA_TTS_MANAGED` documented in `ENV_TEMPLATE` (`envfile.ts`) and whitelisted in
  `WIZARD_KEYS` (`onboarding.ts`).
- **Tests**: NEW `ttsRuntime.test.ts` (14 — loopback parse, flag gating, remote-URL refusal, BYO/
  provisioned precedence, mid-install fallback, venv per-platform, newest-yaml pick, argv shape) + 4
  serve health cases (starting/gave-up 200s, idle/null 502s, reachable-wins-over-restarting).
  `voicePack.test.ts` no-execute contract untouched and green. Suite 1280 → **1299 pass / 0 fail**;
  protocol/server/web/desktop tsc all clean.

Inference:

- This reverses the v0.35.3 never-spawn boundary as a deliberate, flag-gated escape hatch rather than
  a rewrite: `voicePack.ts` stays a pure no-execute scanner/installer (its contract test is the
  proof), and the only thing Luna will ever spawn is her own vetted api_v2 entrypoint — never
  anything derived from a dropped pack.
- The health-state answer is the load-bearing piece for 标准 2 (v0.37.1): it converts "connection
